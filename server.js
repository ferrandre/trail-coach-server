...
app.post('/strava/token', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Errore Strava token' });
  }
});

// Proxy: scarica le attività Strava
app.post('/strava/activities', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Errore Strava activities' });
  }
});
